...
if($_REQUEST['task'] == 'events' && $_REQUEST['p'] == 'edit' && $U['auth_key'])
{
    ...
    if(!$_POST['descr'])
        $MSG .= '<li>Описание мероприятия</li>';
    ...
    if(!$MSG)
    {
        ...
        if($event_id)
        {
            $res = $DB->query("UPDATE `events` SET `title`='$title', `addr`='$addr', `map`='$map', `descr`='$descr', `descr`='$descr', `admin_id`={$U['id']}, `game_id`={$P['game']} WHERE `id`=".$event_id);
        ...
        {
            $res = $DB->query("INSERT INTO `events` (`title`, `descr`, `org_id`, `lang_id`, `dt`, `status`, `game_id`, `city_id`, `price`, `count_min`, `count_max`, `count_free`, `count_paid`, `link`, `admin_id`, `addr`, `map`) VALUES ('$title', '$descr', {$P['org']}, {$P['lang']}, '$dt:00', 0, {$P['game']}, {$P['city']}, {$P['price']}, {$P['count_min']}, {$P['count_max']}, {$P['count_free']}, 0, '', {$U['id']}, '$addr', '$map')");
    ...
    if($event_id)
    {
        if($_FILES['event_img']['error'] == UPLOAD_ERR_OK) // картинка зааплоадена, все ок
        {
            if(dirname($_FILES['event_img']['type']) == 'image')
            {
                $ext = pathinfo($_FILES['event_img']['name'], PATHINFO_EXTENSION);
                $dst = $_SERVER['DOCUMENT_ROOT'].UPLOAD_DIR.'/'.$event_id.'.'.$ext;

                if(move_uploaded_file($_FILES['event_img']['tmp_name'], $dst))
                {
                    if(!$DB->query("UPDATE `events` SET `img_ext`='$ext' WHERE `id`=".$event_id))
...
if($_REQUEST['task'] == 'bot' && $_REQUEST['p'] == 'users' && $U['auth_key'])
{
