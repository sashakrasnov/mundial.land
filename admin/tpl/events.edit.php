...
<?php print_r($e); ?>
              ...
              <div class="row">
                <div class="col form-group">
                  ...
                  <?php else: ?>
                  <textarea name="descr" class="form-control" id="descr" placeholder="Введите описание" rows="5"><?=($e ? htmlentities($e['descr']) : $_POST['descr']);?></textarea>
                  <?php endif; ?>
                </div>
              </div>
              <!-- Ссылка на отчет -->
              ...
              <div class="row">
                <div class="col form-group">
                    <?php if($e && $e['status'] < 0): if($e['img_ext']): ?>
                    <label for="imgDisp">Фотография:</label>
                    <img id="img-upload" src="<?=UPLOAD_DIR.'/'.$e['id'].'.'.$e['img_ext'];?>">
                    <?php endif; ?>
                    <?php else:  ?>
                    <label for="imgDisp">Фотография:</label>
                    <div class="input-group">
                      <div class="input-group-prepend">
                        <span class="input-group-text btn btn-outline-primary btn-file" id="imgDisp">Выбрать…
                            <input type="file" id="imgInp" accept="image/*" name="event_img">
                        </span>
                      </div>
                      <input type="text" class="form-control form-control-file border">
                    </div>
                    <img id="img-upload" <?=($e['img_ext'] ? 'src="'.UPLOAD_DIR.'/'.$e['id'].'.'.$e['img_ext'].'"' : '');?>>
                    <?php endif; ?>
                </div>
              </div>
              <!-- Сохранить/Добавить -->
